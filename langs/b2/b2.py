...
class B2AnnotatedCompiler(Visitor):
    def __init__(self):
        super().__init__(
            {
                "<b2>": self._visit_b2,
    ...
    def compile(self, ast: ASTNode) -> str:
        ...
        for varname in self._ra:
            self._a[varname] = f"r{self._ra[varname]}"
        # todo: why need star tuple?
        preamble: Text = Text(
            *(
                Text.Line(
                    Text.Token("setv"), Text.Token(f"r{i}"), Text.Token(self._ma[i])
                )
                for i in self._ma
            )
        )
        return str(
            Text.join(
                preamble,
                self.visit(ast),
                Text(Text.Line(Text.Token("exitv"), Text.Token("0"))),
            )
        )
    ...
    def _visit_b2(
        ...
        return Text.join(*map(self.visit, n[0]))
    ...
    def _visit_block(
        ...
        match n.choice:
            ...
            # <block> ::= "{" <statements> "}";
            case 1:
                return Text.join(*map(self.visit, n[1])).tabbed()
        ...
        match n.choice:
            ...
            case 0:
                ...
                match n[2].choice:
                        ...
                        return Text(
                            Text.Line(
                                (
                                    Text.Token("set")
                                    if n[2][0].choice == 0
                                    else Text.Token("setv")
                                ),
                                Text.Token(self._a[varname]),
                                # todo: text manipulation -- need to have this return a Text.Token
                                self.visit(n[2][0]),
                            )
                        )
                    ...
                    # <expression>
                    case 1:
                        # todo: text manipulation -- continue from here
                        return self.visit(n[2][0]).format(self._a[varname])
