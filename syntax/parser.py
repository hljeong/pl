...
class Parse:
    ...
    class NBacktracking:
        ...
        NodeParser = Callable[["Parse.NBacktracking"], Result | None]
        ...
        def __init__(
            ...
            self._grammar_name: str = grammar_name
            self._node_parsers: dict[str, Parse.NBacktracking.NodeParser] = node_parsers
            self._entry_point: str = entry_point

        def _parse(self) -> ASTNode:
            parse_result: Parse.NBacktracking.Result | None = self._node_parsers[
                self._entry_point
            ](self)
        ...
        def parse_node(
            self, term: NExpressionTerm, **ctx: Any
        ) -> Parse.NBacktracking.Result | None:
            Log.t(
            ...
            # todo: type annotation
            parse: Parse.NBacktracking.NodeParser = self._node_parsers[term.node_type]
            parse_result: Parse.NBacktracking.Result | None = parse(self, **ctx)
        ...
        @staticmethod
        def generate_nonterminal_parser(
            nonterminal: str, body: list[list[NExpressionTerm]]
        ) -> NodeParser:

            def nonterminal_parser(
                parser: Parse.NBacktracking,
            ) -> Parse.NBacktracking.Result | None:
                choices: dict[int, Parse.NBacktracking.Result] = {}
                ...
                for choice, production in enumerate(body):
                    ...
                    c_res: Parse.NBacktracking.Result | None
                    c: ASTNode

                    for term in production:
                        c_res: Parse.NBacktracking.Result | None = parser.parse_node(
                            term
                        )
                    ...
                    if good:
                        choices[choice] = Parse.NBacktracking.Result(
                            n, n_tokens_consumed
                        )
                    parser._backtrack(save)
        ...
        def generate_alias_parser(
            ...
            def alias_parser(
                ...
                if token is None:
                    return None
                return Parse.NBacktracking.Result(
                    AliasASTNode(alias, terminal, token), 1
                )
        ...
        def generate_terminal_parser(
            ...
            # todo: review epsilon handling
            if terminal == "e":
                return lambda *_: Parse.NBacktracking.Result(
                    TerminalASTNode(terminal, Token("e", "")), 0
                )
            ...
            def terminal_parser(
                ...
                if token is None:
                    return None
                return Parse.NBacktracking.Result(TerminalASTNode(terminal, token), 1)
        ...
        def generate_parsers_from_vocabulary(
            ...
            return {
                terminal: Parse.NBacktracking.generate_terminal_parser(terminal)
                for terminal in vocabulary
            }
