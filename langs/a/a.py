...
with open("langs/a/spec/a.xbnf") as a_xbnf_f:
    a_xbnf: str = a_xbnf_f.read()

a_grammar: Grammar = Grammar("a", a_xbnf, ignore=[f"#[^\n]*"])
...
class AParse:
    def __call__(self, prog, entry_point: Optional[str] = None) -> ASTNode:
        return (
            Monad(prog)
            .then(Lexer(a_grammar))
            .then(Parser(a_grammar, entry_point=entry_point))
            .value
        )


class APrint(Visitor):
    def __init__(self):
        super().__init__(
...
class ABuildInternalAST(Visitor):
    def __init__(self):
        super().__init__(
...
class ATranslate(Visitor):
    ...
    def _visit_pseudoinstruction(self, n: ASTNode) -> tuple[ASTNode]:
        ...
        def ins(code: str, extras: dict[str, Any] = {}) -> ASTNode:
            n_: ASTNode = AParse()(code, entry_point="<instruction>")[0]
        ...
        match n.choice:
            ...
            # todo: big vals
            case 10:
                return (ins(f"addi {n[1][0].lexeme} zr {n[2][0].lexeme}", n.extras),)
...
class AAssemble(Visitor):
    ...
    def __init__(self) -> None:
        super().__init__(
            default_nonterminal_node_visitor=AAssemble._combine,
            default_terminal_node_visitor=lambda *_, **ctx: Prog(),
        )

    def _visit_a(self, n: ASTNode) -> Prog:
        label_map: dict[str, int] = AAssemble.ResolveLabels()(n)
        return AAssemble._combine(self, n, label_map=label_map)
    ...
    def _resolve(self, n: ASTNode, label_n: ASTNode, label_map: dict[str, int]) -> int:
        return label_map[label_n[0].lexeme] - n.extras["loc"]

    def _visit_pseudoinstruction(self, n: ASTNode, label_map: dict[str, int]) -> Ins:
        def ins(code: str) -> ASTNode:
            n_: ASTNode = AParse()(code, entry_point="<instruction>")
            return n_
        ...
        match n.choice:
            ...
            case 3:
                return self(
                    ins(f"bne {n[1][0].lexeme} zr {self._resolve(n, n[3], label_map)}")
                )
            ...
            case 11:
                return self(
                    ins(f"addi {n[1][0].lexeme} pc {self._resolve(n, n[2], label_map)}")
                )
